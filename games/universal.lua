    ...
    ChangeLogs = vape.Categories.Minigames:CreateModule({
        ...
        Function = function(callback)
			...
			local function CreateLogs()
				local Profile = createlib:Create("ScreenGui", {
					Name = "Logs",
					DisplayOrder = 30,
					ResetOnSpawn = false,
				...
				local BackgroundProfileUI = createlib:Create("ImageButton", {
					Name = "Background",
				...
				local MainProfileFrame = createlib:Create("Frame", {
					Name = "Main",
					Parent = Profile,
				...
				local MainMainBG = createlib:Create("ImageButton", {
					Name = "MainBG",
					AutoButtonColor = false,
				...
				createlib:Create("UIAspectRatioConstraint", { Parent = MainMainBG, AspectRatio = 1.143 })
				createlib:Create("UIScale", { Parent = MainMainBG, Scale = 1.297 })
		
				local IconButtonWrapper = createlib:Create("ImageButton", {
				...
				createlib:Create("UICorner", { CornerRadius = UDim.new(1, 0), Parent = IconButtonWrapper })
				createlib:Create("UIPadding", {
				...
				local FrameMainBG = createlib:Create("Frame", {
					Parent = MainMainBG,
					BackgroundColor3 = Color3.fromRGB(100, 103, 167),
					Size = UDim2.fromScale(1, 1)
				})
				createlib:Create("UICorner", { CornerRadius = UDim.new(0.05, 0), Parent = FrameMainBG })
				createlib:Create("UIListLayout", {
					Parent = FrameMainBG,
					FillDirection = Enum.FillDirection.Vertical,
					SortOrder = Enum.SortOrder.LayoutOrder
				})
		
				local UserFrame = createlib:Create("Frame", {
					Name = "UserFrame",
					Size = UDim2.fromScale(1, 1),
					BackgroundColor3 = Color3.fromRGB(78, 80, 130),
					Parent = FrameMainBG
				})
				createlib:Create("UICorner", { CornerRadius = UDim.new(0, 8), Parent = UserFrame })
		
				local BGImage = createlib:Create("ImageLabel", {
					Name = "BGImage",
					Parent = UserFrame,
					BackgroundTransparency = 1,
...
run(function()
	local UGR
	-- adding ltr wildkarduno, elijxhwtf, thr33, smokingskul, luracks, tdf,perc40,19thou,okaymar,ohsxnta,osamason
	local list = {"killjae", "1oneam",'boolymon','xtrovert'}
	local Notifys
	local Volume
	local List
	local song 
	local function GetSongNameNoExt(path)
    	return path:match("([^/\\]+)%.%w+$")
	end
	local function RandomSong(Rapper)
		local files = listfiles("ReVape/sounds/"..Rapper)
		local Song = files[math.random(1, #files)]
		return Song 
	end
	local function playRandomSong()
		local old = workspace:FindFirstChild(List.Value)
		if old then
			old:Destroy()
			song = nil
		end
		local ID = RandomSong(List.Value)
		local NAME = GetSongNameNoExt(ID)
		song = Instance.new("Sound")
		song.Volume = Volume.Value
		song.Name = List.Value
		song.Parent = workspace
		song.SoundId = assetfunction(ID)
		if Notifys.Value then
			vape:CreateNotification("Onyx", "Now playing, "..NAME.."!", 12)
			song:Play()
		else
			song:Play()
		end
	...
	UGR = vape.Legit:CreateModule({
		Name = "UndergroundRappers",
		Tooltip = 'Plays random songs from these rappers',
		Function = function(callback)
			if vape.role ~= "owner" and vape.role ~= "coowner" and vape.role ~= "admin" and vape.role ~= "friend" and vape.role ~= "premium"and vape.role ~= "user"then
			...
			if callback then
				...
				workspace:FindFirstChild(List.Value):Destroy()
			end

			
		end,
	})
	List = UGR:CreateDropdown({
		Name = "Rappers",
		List = list,
		Darker = true
	})
	Notifys = UGR:CreateToggle({
		Name = "Notifications",
		Default = true,
		Darker = true,
		Tooltip = 'This just makes a notification whenever a new song is playing'
	})
	Volume = UGR:CreateSlider({
		Name = "Volume",
		Default = 1,
		Darker = true,
		Min = 0,
...
run(function()
	...
	Spotify = vape.Legit:CreateModule({
		Name = "Spotify",
		Tooltip = 'a custom spotify player',
		Function = function(callback)
			if vape.role ~= "owner" and vape.role ~= "coowner" and vape.role ~= "admin" and vape.role ~= "friend" and vape.role ~= "premium"and vape.role ~= "user"then
				vape:CreateNotification("Onyx", "You do not have permission to use this", 10, "alert")
				return
